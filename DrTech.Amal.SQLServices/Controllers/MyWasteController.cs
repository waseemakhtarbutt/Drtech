...
namespace DrTech.Amal.SQLServices.Controllers
{
    ...
    public class MyWasteController : BaseController
    {
        ...
        public async Task<ResponseObject<List<City>>> GetAllCity()
        {
            try
            {
                List<City> lstCity = db.Repository<City>().GetAll().ToList();
        ...
        public async Task<ResponseObject<List<Area>>> GetAreaByID(int ID)
        {
            try
            {
                List<Area> lstArea = db.Repository<Area>().GetAll().Where(x => x.CityID == ID).ToList();
        ...
        public ResponseObject<bool> AddWasteSchedule(List<MSchedule> lstSchedule)
        {
            ...
            try
            {
                ...
                List<WetWasteSchedule> ExitData = db.Repository<WetWasteSchedule>().GetAll().Where(x => x.CreatedBy == UserID && x.CityID == schedule.CityID && x.AreaID == schedule.AreaID && x.Month == schedule.Date.Month && x.Year == schedule.Date.Year).ToList();
                if (ExitData.Count > 0)
                {
        ...
        [HttpGet]
        public async Task<ResponseObject<List<object>>> GetWetWasteSchedule(int AreaID, int CompanyID)
        {
            try
            {
                List<object> lstSchdule = db.ExtRepositoryFor<MyWasteRepository>().GetScheduleDetails(AreaID, CompanyID).ToList();
